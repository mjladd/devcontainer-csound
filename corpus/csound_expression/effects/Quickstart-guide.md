...
file: csound_expression_wiki/Quickstart-guide.md
category: Effects
concepts: [osc, envelope, filter, reverb, distortion, midi, random, spectrum]
type: tutorial
---
...
**Category:** Effects
**Concepts:** osc, envelope, filter, reverb, distortion, midi, random, spectrum
**Original File:** `csound_expression_wiki/Quickstart-guide.md`
...
~~~
Prelude Csound.Base> let effect x = return $ 0.5 * nreverb x 1 0.2
Prelude Csound.Base> vdac $ effect =<< midin 1 (instr osc) + midin 2 (instr saw)
~~~
...
The most simple event stream is created with the function `metro`:
...
~~~
Prelude Csound.Base> let notes = fmap (const 440) $ metro 2
~~~
...
~~~
Prelude Csound.Base> let notes = fmap (const (440::D)) $ metro 2
Prelude Csound.Base> :t notes
notes :: Evt D
~~~
...
~~~
Prelude Csound.Base> let notes = cycleE [440::D, 330, 220] $ metro 4
~~~

Or we can play them at random and skip some elements with the inverse of the given frequency:

~~~
Prelude Csound.Base> let notes = randSkip 0.75 $ oneOf [440::D, 330, 220] $ metro 4
~~~
