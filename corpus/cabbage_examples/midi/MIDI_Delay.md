...
category: MIDI
file: examples/MIDI/MIDI_Delay.csd
title: "Delay Mode"
type: cabbage_example
widgets: [form, label, combobox, image, rslider, nslider]
---

# Delay Mode

**Category:** MIDI
**Source File:** `examples/MIDI/MIDI_Delay.csd`
**Cabbage Widgets:** form, label, combobox, image, rslider, nslider

## Description

MIDI_Delay.csd Written by Iain McCurdy, 2015 Note that the delay needs to be set up at i-time so changes to parameters force the delay to clear and restart Delay Mode  -    method how the delay time is derived: 1. from a Time (s) dial 2. from a metrical value derived from Tempo and a note value fraction Time (s)    -    delay time (initial delay time if 'Warp' is anything other than '1') Tempo (BPM) -    if Delay mode is Tempo this is used as the base for the delay time Note fraction - a fractio
