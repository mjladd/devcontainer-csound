...
category: Effects
file: examples/Effects/Spectral/PitchShifter.csd
title: "Interval"
type: cabbage_example
widgets: [form, label, combobox, image, nslider, rslider, button]
---

# Interval

**Category:** Effects
**Source File:** `examples/Effects/Spectral/PitchShifter.csd`
**Cabbage Widgets:** form, label, combobox, image, nslider, rslider, button

## Description

PitchShifter.csd Iain McCurdy, 2015. A pitch shifter using the traditional delay buffer write-read method. Two buffers are used per channel (therefore four in total). Each overlapping buffer is gated using a hanning window in order to prevent discontinuities Snap        -    when pressed forces 'Semitones' to an integer. Three modes are provided: Feedback    -    a standard single pitchshifter unit with feedback (2 feedback modes are offered) Interative  -    the user can choose the number of si
