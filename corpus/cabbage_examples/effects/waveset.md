...
category: Effects
file: examples/Effects/Time/waveset.csd
title: "Repeats"
type: cabbage_example
widgets: [form, image, rslider, checkbox, line, button]
---

# Repeats

**Category:** Effects
**Source File:** `examples/Effects/Time/waveset.csd`
**Cabbage Widgets:** form, image, rslider, checkbox, line, button

## Description

waveset.csd 'freeze' is not technically a freeze but instead a very large number of repeats. Waveset opcode can be reinitialised by three different methods: Manually, using the 'Reset' button, by a built-in metronome, the rate of which can be adjusted by the user or by the dynamics of the input sound (the threshold of this dynamic triggereing can be adjusted by the user) 'Metro' resetting is disabled when 'Metro Rate' = 0 'Threshold' (retrigering by input signal dynamics) is disabled when 'Thres
