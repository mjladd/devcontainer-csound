...
category: Effects
file: examples/Effects/Time/BreakBeatCutter.csd
title: "CUTTER"
type: cabbage_example
widgets: [form, groupbox, rslider, hslider, label, button]
---

# CUTTER

**Category:** Effects
**Source File:** `examples/Effects/Time/BreakBeatCutter.csd`
**Cabbage Widgets:** form, groupbox, rslider, hslider, label, button

## Description

BreakBeatCutter.csd Iain McCurdy, 2013. Break Beat Cut Up using the bbcut opcode with additional processing 'Sub-division' determines the note duration used as the base unit in  cut-ups. For example a value of 8 represents quavers (eighth notes), 16 represents semiquavers (sixteenth notes) and so on. 'Bar Length' represents the number of beats per bar. For example, a value of 4 represents a 4/4 bar and so on. 'Phrase' defines the number of bars that will elapse before the cutting up pattern rest
