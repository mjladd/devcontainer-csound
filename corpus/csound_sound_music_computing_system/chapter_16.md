...
with the fundamental frequency. The code is ﬁnished with the FINE keyword, after
...
10
FINE
MUSIC IV was the ﬁrst fully ﬂedged computer music programming environ-
...
of the ﬁrst scientiﬁc high-level programming languages. An important feature of
this version was that it also used the FORTRAN language for the programming of
instrument deﬁnitions.
1.2.2 MUSIC V
Mathews’ work at Bell Labs culminated in MUSIC V [84], written in collaboration
with Joan Miller, Richard Moore and Jean-Claude Risset [52]. This was the ﬁnal it-
...
or as separate machine-language subroutines, which could be optimised for speciﬁc
computers.
...
Gfortran compiler [16]. This software represents a signiﬁcant milestone in Com-
...
NOT in the score deﬁnes an oscillator instance, with the ﬁrst three parameters (P2,
P3, P4) deﬁned as start time (0), instrument (1), and duration (14). Oscillator fre-
quencies are deﬁned by sampling increments (in P6 and P7). The top frequency of
the decaying exponential is P6×fs/512, where fs is the sampling rate. The ampli-
tude and frequency envelopes have a cycle that lasts for 512/( fs× P7).
Pass I of MUSIC V scans the score (which includes both the instrument deﬁni-
...
Listing 1.2 MUSIC V, Risset-Shepard tones, from Risset’s catalogue [108]
...
combined into a single ‘load module’ (the program) after the orchestra is compiled.
In many ways, MUSIC 360 represented a signiﬁcant advance with regards to MU-
SIC IV. It allowed any number of parallel instances of instruments to be performed
at the same time. An important innovation seen in this system is the clear deﬁnition
...
For instance, let’s say we want to notate the pitch of the sound using a system
...
addition operators, and separate multipliers, redundant. Data types included ‘alpha’-
...
hold program constants), P-types for note list p-ﬁelds, and U-types, which could
be used to reference unit generator results (as an alternative to ‘alpha’ variables).
...
also supported conditional control of ﬂow, another advanced feature when compared
...
previously deﬁned in the code. In this case U1 is a reference to the unit one line
above the current. The PSAVE statement is used to indicate which p-ﬁelds from
score cards will be required in this instrument. The ISIPCH converter is used to
convert “octave.pitch class” notation into a suitable sampling increment, operating
...
crement is a low-level way of deﬁning the frequency of an oscillator, and we will
explore its deﬁnition later in this book.
...
which is deﬁned as a score statement. The syntax is very close to classic Csound
...
sic Studio. As the PDP 11’s popularity grew in the 1970s, and with the introduction
...
were pioneered here.
One of the main design aspects ﬁrst introduced in MUSIC 11 was the concept
of control (k-) and audio (a-) computation rates. This is a further reﬁnement of the
initialisation- and performance-time optimisation principle. This made the system
the most computationally efﬁcient software for audio synthesis of its time. It es-
...
introduced in the language. The header declaration now includes the deﬁnition of
...
The type system has been simpliﬁed; we observe the presence of the k- and a-
type variables, which have now been deﬁned to hold signals (and not just references
...
between efﬁciency and precision needs to be reached, where the ratio between au-
...
and perf. The ﬁrst command would sort the score; the second would compile and
1 Despite the different ways in which its name is written down in various places, there is only
one correct form: capital ‘C’ followed by lowercase ‘sound’. A fully lowercase variant csound is
...
Csound 6
2013
Fig. 1.1 The Csound family tree, from MUSIC IV through to the ﬁrst release of the MIT-EMS
...
MIT-EMS version. From this, the system developed into an ofﬂine composition and
...
UNIX sources modiﬁed for the THINK C compiler.
14
1 Music Programming Systems
...
launched in 2006, twenty years after the ﬁrst MIT-EMS release in 1986. From a lan-
guage perspective, there were a few signiﬁcant changes from the original Csound,
which brought more ﬂexibility to programming. A plug-in mechanism made it sim-
...
language. User-deﬁned unit generators were introduced, providing further facilities
for structuring the code.
A signiﬁcant innovation in version 5 is the presence of an Application Program-
...
1.3.2 Csound 6
The next major version of the system, Csound 6, was ﬁrst released in 2013. The code
...
The compiler was signiﬁcantly modiﬁed allowing it to operate in an on-the-ﬂy
mode, adding new instruments on demand to a running system. Such changes were
also reﬂected in a newly designed API, which gave developers increased ﬂexibility
...
embedded systems.
As we have seen in this introduction to music programming systems, Csound ﬁts
...
with ﬁxed instruments that expose some of their parameters for the user to manipu-
...
During its thirty-odd years of development, Csound has been gifted a signiﬁcant
...
for the MIT-EMS Csound from running in the latest version. There is a signiﬁcant
...
to preserve. Hopefully, this issue is not signiﬁcant, as through education and doc-
...
We have seen how the early music languages evolved from the ﬁrst direct synthesis
...
360 and MUSIC 11, which were created for speciﬁc computers. Other classic sys-
tems of note were MUSIC V, written in FORTRAN, which inﬂuenced the design of
...
in computer applications. We have shown that there are many ways to ‘attack’ the
